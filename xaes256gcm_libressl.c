    ...
    size_t ciphertext_max_len)
{
	...
	if (!EVP_AEAD_CTX_seal(c->gctx, ciphertext, &outlen, ciphertext_max_len,
		nonce, EVP_AEAD_nonce_length(aead), plaintext, plaintext_len,
		aad, aad_len))
		goto err;
...
{
	/* Using RAND_bytes for portability.
	 * It's deprecated but kept for compatibility,
	 * and uses arc4random_buf(3) on OpenBSD.
	 */
...
{
	/* Using OPENSSL_cleanse for portability.
	 * It's deprecated but kept for compatibility,
	 * and uses explicit_bzero(3) on OpenBSD.
	 */
